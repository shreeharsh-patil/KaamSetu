...
export function VoiceAssistantFAB() {
  ...
  return <>
    <button type="button" onClick={() => setIsOpen(true)} aria-label="Book by voice" className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] md:bottom-6 right-4 sm:right-6 z-50 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-gradient-to-tr from-primary to-orange-500 text-white shadow-2xl"><Mic className="h-5 w-5 sm:h-6 sm:w-6" /></button>
    {isOpen ? <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-background/80 p-0 sm:p-4 backdrop-blur-sm"><div className="w-full max-w-lg max-h-[88dvh] overflow-y-auto overscroll-contain rounded-t-3xl sm:rounded-3xl border bg-card shadow-2xl p-4 pb-[max(1rem,env(safe-area-inset-bottom))] space-y-4">
